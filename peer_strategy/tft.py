...
class TFT(Peer):
    def __init__(self, sim_config, ID=0, strategy = 'TFT'):
        super().__init__(sim_config, ID, strategy)
        self.num_history = sim_config['TFT_history']

    
    def tft_strategy(self, neighbor_leecher_list, unchoke_num):
        #参照する履歴数を決定する
        ...
        num_upload_peer = 0
        choose = []
        #ref は基準値. アップロードが多い順に取っていく. 0(ダウンロードしていないピア)は取らない
        for ref in range(max(sum_download), 0, -1):
        ...
        #print(self.ID,':\t', ':', sum_download, choose)
        return choose
    ...
    def upload(self, status_list, random_seed = random.randint(0, 1000000)):
        ...
        if self.is_seeder() or self.first_action:
            self.first_action = False
            #隣接リーチャの中からランダムに相手を選択.
            return self.seeder_strategy(neighbor_leecher_list, unchoke_num)
        else:
            return self.tft_strategy(neighbor_leecher_list, unchoke_num)
